...
Sessions are automatic: the kit exchanges this token for a proxy-issued
**7-day pass** on first call, so Google's ~1-hour expiry never interrupts a
run and users re-login only after 7 days. (For processes that stay open for
days you may pass a token-refreshing **function** instead of the string —
details in the `pw_access.py` header.)
...
resp = pw_access.gemini_generate(
    ...
    count=20,
    video_duration="03:42",          # optional; video apps only, column L
)
...
For non-video apps, omit `video_duration`; column L stays blank.
